...
services.map((service) => {
  ...
  const serv = new gcp.cloudrun.Service(service, {
    location: 'europe-west1',
    name: service,
    template: {
      spec: {
...
try {
  const psToAvro = new gcp.dataflow.Job('ps-to-text-be-logs', {
    ...
    parameters: {
      ...
      outputDirectory: `${rawBucket.url}/text`,
      // outputFilenamePrefix: "my-shit"
